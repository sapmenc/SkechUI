...
const InspirationSidebar=({isOpen,onClose}:InspirationSidebarProps)=>{
    ...
    //send query to get inspiration images. we need upload url for that images
    ...
    const handleFileSelect=useCallback((files:FileList | null)=>{
       //filter the user input as image
    ...
    //load the existing images from the server when the component mounts
    useEffect(()=>{
      //add server images with any local changes
    ...
    const clearAllImages=async()=>{
        //clear from server we have to clear image indivitually
         const imageToRemove=images.filter((img)=>img.storageId && img.isFromServer)
        //loop through all the images and if it has storageId then call the funtion
    ...
    const removeImage=async(imageId:string)=>{
       //find the image . if it's a server image remove from convex then remove from local state
    ...
    return (
        <div className={
            cn('fixed left-5 top-1/2 transform -translate-y-1/2 w-80 backdrop-blur-xl bg-white/8 border-white/12 gap-2 p-3 saturate-150 border rounded-lg z-50 transition-transform duration-300')
        }>
          ...
          <div className={
                cn(
                  ...
                  dragActive ? 'border-blue-400 bg-blue-500/10':images.length < 6 ? 'border-white/20 hover:border-white/40 hover:bg-white/5':'border-white/10 bg-white/5 cursor-not-allowed opacity-50'
                )}
            ...
            images.length > 0 && (
               <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Button className="h-7 px-2 text-xs text-white/60 hover:text-white hover:bg-white/10"
                    variant="ghost"
                    size="sm"
